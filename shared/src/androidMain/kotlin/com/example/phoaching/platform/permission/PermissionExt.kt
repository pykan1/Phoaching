...
import com.example.phoaching.platform.permission.util.CannotOpenSettingsException
...
/**
 * Функция для открытия страницы настроек приложения для конкретного действия.
 * @param action Действие, для приложения, с помощью которого можем открыть нужный экран в настройках.
 * @throws CannotOpenSettingsException Если не удается открыть страницу настроек.
 * при переходе используем пакет нашего приложения
 */
internal fun Context.openActionSettingsPage(action: String) {
    openPage(
        action = action,
        newData = Uri.parse("package:$packageName"),
        onError = { throw CannotOpenSettingsException(action) }
    )
}
